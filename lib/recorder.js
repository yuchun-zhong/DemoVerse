...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  try {
    ...
    console.error('标题页生成失败:', err.message);
  }
}
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  try {
    // 如果当前页面已失效（detached frame），打开新页面
    let targetPage = page;
    try {
    ...
    await targetPage.goto(`file://${endingPath}`, { waitUntil: 'networkidle0', timeout: 10000 });
    await delay(500);

    // 捕获结尾页帧（4秒展示）
    for (let i = 0; i < 8; i++) {
      await frameCapture.capture(targetPage, 0.5);
    }
  } catch (err) {
    console.error('结尾页生成失败，跳过:', err.message);
    // 结尾页失败不影响主流程，跳过即可
  }
}
